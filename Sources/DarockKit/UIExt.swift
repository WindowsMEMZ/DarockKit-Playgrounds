...
// Applied some changes to fit Swift Playgrounds
...
@ViewBuilder public func NeuButton(action: @escaping () -> Void, label: () -> AnyView, cornerRadius: CGFloat = 18, paddings: [CGFloat] = [5, 20], fontSize: CGFloat = 22, fontWeight: Font.Weight = .bold, width: CGFloat? = nil) -> some View {
    Button(action: {
        action()
    }, label: {
        label()
...
@ViewBuilder public func NStack(_ main: () -> AnyView) -> some View {
    ZStack {
...
@ViewBuilder public func NeuToggle(isOn: Binding<Bool>, label: () -> AnyView, tint: Color = .green) -> some View {
    HStack {
        label()
        Toggle("", isOn: isOn)
            .softSwitchToggleStyle(tint: tint, labelsHidden: true)
...
@ViewBuilder public func NeuNavigationLink(destination: () -> AnyView,  label: () -> AnyView, cornerRadius: CGFloat = 18, paddings: [CGFloat] = [5, 20], width: CGFloat? = nil) -> some View {
